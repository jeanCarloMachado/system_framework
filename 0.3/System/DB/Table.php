	...
	class System_DB_Table
	{
		...
		*/
			...
			if($this->hasTableName())
			{
				...
				foreach($array as $columnName => $columnValue)
				{
					...
					foreach($tableSchema as $idColumnSchema => $valuesSchema)
					{
						...
						{
							$hasColumn = true;
							if(substr($valuesSchema['Type'],0,-4) == 'int' || substr($valuesSchema['Type'],0,-3) == 'int' ||  substr($valuesSchema['Type'],4,-3) == 'int' )
		...
		{	
			...
			$obj = System_DesignPattern_Factory_Factory::initialize($fileName,array('query'=>$this->query,
													  			'db' => $this->_db));
