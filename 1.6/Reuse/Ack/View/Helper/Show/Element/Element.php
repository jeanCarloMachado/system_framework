<?php
    ...
    class Reuse_Ack_View_Helper_Show_Element
    {
    	 ...
    	 */
    	public static function input(System_Var $columObj,$title,$permissionLevel=null,$defVal="",$helperId=null)
    	{
    		...
    		?>
    			   	<fieldset>
                        <legend>
                                    ...
                                    <?php if(!empty($helperId)) {?>
                                   	<button id="p_<?= $helperId ?>" class="ajuda icone">(?)</button> 
                                   	<?php } ?>
                       </legend>
                        <input type="text" <?php if($permissionLevel > 1) { ?>name="<?= $columObj->getColumnName() ?>" <?php } ?> value="<?= (self::getAction() == "incluir" )? $defVal : $columObj->getVal() ?>" <? if ($permissionLevel < self::LOWEST_EDITABLE_PERMISSION) { ?>disabled="disabled"<? } ?> />
                    </fieldset>
    		<?php 
    	}
    	 ...
    	 * @param string $permissionLevel
    	 */
    	public static function radioBoolean(System_Var $columObj,$title,$permissionLevel=null,$helperId =null)
    	{
    		$title = ($title) ? $title : "Título não definido";
    		$permissionLevel = ($permissionLevel) ? $permissionLevel : self::getPermissionLevel();
		...
		public static function language() 
		{
			...
			?>
			    	...
			    	<div class="legend"><em>Idioma</em><button class="ajuda icone" id="p_296">(?)</button></div>
