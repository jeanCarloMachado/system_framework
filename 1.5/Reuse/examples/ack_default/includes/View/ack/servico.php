...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <? include_once('_header.php'); ?>
            ...
            <div id="breadcrumbs">
            	<a href="<? echo $endereco_site; ?>/ack/servicos" class="btnSeta voltarTopo" title="Voltar">Voltar</a>
            	<ul>
                	<li><a href="<? echo $endereco_site; ?>/ack/servicos" title="Institucional"><em>Serviços</em></a></li>
                    <li><span><em><? if ($tipoAcao=="incluir") { ?>Adicionar<? } ?><? if ($tipoAcao=="editar") { ?>Editar<? } ?> serviço</em></span></li>
                </ul>
            </div><!-- END breadcrumbs -->
            ...
            <div id="descricaoPagina">
            	<h2><? if ($tipoAcao=="incluir") { ?>Adicionar<? } ?><? if ($tipoAcao=="editar") { ?>Editar<? } ?> serviço</h2>
				<?
            ...
            <div class="parentFull servico">
            	<input type="hidden" class="dadosPagina" id="servicos" <? if ($tipoAcao=="editar") { ?>value="<? echo $dadosServico["id"]; ?>"<? } ?> />
                
                <div class="modulo servicos">
                	<div class="head">
                    ...
                    <div class="slide colunas">
                    	<div class="collumA form">
                            <fieldset>
                                <legend><em>Nome do serviço </em><strong>[<? echo $conteudoIdiomas[0]["nome"]; ?> - <? echo strtoupper($conteudoIdiomas[0]["abreviatura"]); ?>]</strong></legend>
                                <input type="text" <? if ($nivelPermissao["nivel"]=="2") { ?>name="titulo_<? echo $conteudoIdiomas[0]["abreviatura"]; ?>"<? } else { ?>disabled="disabled"<? } ?> value="<? echo $dadosServico["titulo_".$conteudoIdiomas[0]["abreviatura"]]; ?>" />
                            </fieldset>
                            ...
                            <fieldset class="editorTexto textarea683x110">
                                ...
                                <textarea id="editor" <? if ($nivelPermissao["nivel"]=="2") { ?>name="descricao_<? echo $conteudoIdiomas[0]["abreviatura"]; ?>"<? } ?> rows="5" cols="50"><? echo $dadosServico["descricao_".$conteudoIdiomas[0]["abreviatura"]]; ?></textarea>
                            </fieldset>
                        </div><!-- END collumA -->
                        ...
                        <div class="collumB">
                            <fieldset class="radioGrup checkVisivel">
                            	...
                            	<label><input type="radio" <? if ($nivelPermissao["nivel"]=="2") { ?>name="visivel"<? } else { ?>disabled="disabled"<? } ?> value="1"<? if ($dadosServico["visivel"]=="1" or !$dadosServico) {?> checked="checked"<? } ?> /><span>Sim</span></label>
                            	<label><input type="radio" <? if ($nivelPermissao["nivel"]=="2") { ?>name="visivel"<? } else { ?>disabled="disabled"<? } ?> value="0"<? if ($dadosServico["visivel"]=="0") {?> checked="checked"<? } ?> /><span>Não</span></label>
                                    ...
                                    <ul>
										<?
                                            if (count($categorias)>0) {
												$categoriasServico=unserialize($dadosServico["categorias"]);
                                                foreach ($categorias as $categoria)	{
													if (array_key_exists($categoria["id"], $categoriasServico)) {
														?>
                ...
                <div class="modulo<? if ($nivelMetatags["nivel"]=="2") { ?> metaTags<? } ?>"<? if ($tipoAcao=="incluir") { ?> style="display:none;"<? } ?>>
                	<div class="head">
            ...
            <div id="footerPage">
            	<a href="<? echo $endereco_site; ?>/ack/servicos" class="btnSeta voltarTopo" title="Voltar">Voltar</a>
                <button class="botao salvar" id="salvarServico" name="<? echo $tipoAcao; ?>" title="Salvar"><span><var></var><em>Salvar</em></span><var class="borda"></var></button>
            </div><!-- END footerPage -->
