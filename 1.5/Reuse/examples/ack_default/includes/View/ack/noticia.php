...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <? include_once('_header.php'); ?>
            ...
            <div id="breadcrumbs">
            	<a href="<? echo $endereco_site; ?>/ack/noticias" class="btnSeta voltarTopo" title="Voltar">Voltar</a>
            	<ul>
                	<li><a href="<? echo $endereco_site; ?>/ack/noticias" title="Institucional"><em>Notícias</em></a></li>
                    <li><span><em><? if ($tipoAcao=="incluir") { ?>Adicionar<? } ?><? if ($tipoAcao=="editar") { ?>Editar<? } ?> notícia</em></span></li>
                </ul>
            </div><!-- END breadcrumbs -->
            ...
            <div id="descricaoPagina">
            	<h2><? if ($tipoAcao=="incluir") { ?>Adicionar<? } ?><? if ($tipoAcao=="editar") { ?>Editar<? } ?> notícia</h2>
				<?
            ...
            <div class="parentFull noticia">
            	<input type="hidden" class="dadosPagina" id="noticias"<? if ($tipoAcao=="editar") { ?>value="<? echo $dadosNoticia["id"]; ?>"<? } ?> />
                ...
                <div class="modulo noticias">
                    ...
                    <div class="slide colunas">
                    	<div class="collumA form">
                            <fieldset>
                                <legend><span>Título da Notícia </span><strong>[<? echo $conteudoIdiomas[0]["nome"]; ?> - <? echo strtoupper($conteudoIdiomas[0]["abreviatura"]); ?>]</strong></legend>
                                <input type="text" <? if ($nivelPermissao["nivel"]=="2") { ?>name="titulo_<? echo $conteudoIdiomas[0]["abreviatura"]; ?>"<? } else { ?>disabled="disabled"<? } ?> value="<? echo $dadosNoticia["titulo_".$conteudoIdiomas[0]["abreviatura"]]; ?>" />
                            </fieldset>
                            
                            <fieldset>
                                <legend><span>Data da Notícia</span> <small>Ex: Dia-Mes-Ano</small></legend>
                                <input type="text" <? if ($nivelPermissao["nivel"]=="2") { ?>name="data"<? } else { ?>disabled="disabled"<? } ?> maxlength="10" value="<? if ($dadosNoticia["data"]) { echo convertDate($dadosNoticia["data"], "%d-%m-%Y"); } ?>" />
                            </fieldset>
                            ...
                            <fieldset class="editorTexto textarea683x110">
                                ...
                                <textarea id="editor" <? if ($nivelPermissao["nivel"]=="2") { ?>name="descricao_<? echo $conteudoIdiomas[0]["abreviatura"]; ?>"<? } ?> rows="5" cols="50"><? echo $dadosNoticia["descricao_".$conteudoIdiomas[0]["abreviatura"]]; ?></textarea>
                            </fieldset>
                        </div><!-- END collumA -->
                        ...
                        <div class="collumB">
                            <fieldset class="radioGrup checkVisivel">
                            	...
                            	<label><input type="radio" <? if ($nivelPermissao["nivel"]=="2") { ?>name="visivel"<? } else { ?>disabled="disabled"<? } ?> value="1"<? if ($dadosNoticia["visivel"]=="1" or !$dadosNoticia) {?> checked="checked"<? } ?> /><span>Sim</span></label>
                            	<label><input type="radio" <? if ($nivelPermissao["nivel"]=="2") { ?>name="visivel"<? } else { ?>disabled="disabled"<? } ?> value="0"<? if ($dadosNoticia["visivel"]=="0") {?> checked="checked"<? } ?> /><span>Não</span></label>
                                    ...
                                    <ul>
                                    	<?
                                            if (count($categorias)>0) {
												$categoriasNoticia=unserialize($dadosNoticia["categorias"]);
                                                foreach ($categorias as $categoria)	{
													if (array_key_exists($categoria["id"], $categoriasNoticia)) {
														?>
            ...
            <div id="footerPage">
            	<a href="<? echo $endereco_site; ?>/ack/noticias" class="btnSeta voltarTopo" title="Voltar">Voltar</a>
                <button class="botao salvar" id="salvarNoticia" name="<? echo $tipoAcao; ?>" title="Salvar"><span><var></var><em>Salvar</em></span><var class="borda"></var></button>
            </div><!-- END footerPage -->
