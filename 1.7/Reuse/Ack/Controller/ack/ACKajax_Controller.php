...
class ACKajax_Controller extends System_Controller
{
	...
	function carregar_mais($dadosJSON) {		
		...
		switch($modulo){
			...
			case 'categoriastags':
				...
				$result = $modelCategorys->toObject()->onlyNotDeleted()->get(null,array("limit"=>array("offset"=>$itens,"count"=>$limite),"order"=>" title_pt DESC"));
				...
				foreach($result as $element)
				{
					...
					$arr["grupo"][] = array(
							"id"=>$element->getId()->getVal(),
							"titulo" =>$element->getTitlePt()->getVal(),
							"visivel"=>1,
							"ordem"=> 0,
							"relacao"=>0
					);
			...
			case 'planos':
					...
					$modelPlans = new CreditPlans();
					$resultPlans = $modelPlans->toObject()->onlyNotDeleted()->get(null,array("limit"=>array("offset"=>$itens,"count"=>$limite),"order"=>" credits_total DESC"));
					
			
					
					$arr["grupo"] = array();
					foreach($resultPlans as $plan)
					{
						$arr["grupo"][] = array(
								"id"=>$plan->getId()->getVal(),
								"total_creditos"=>$plan->getCreditsTotal()->getVal(),
								"total_preco"=>$plan->getPriceTotal()->getVal(),
						);
					}
					
				
					if (count($resultPlans) < $limite){
						$arr['exibir_botao'] = 0;
					} else {
						$arr['exibir_botao'] = 1;
					}
				...
				case 'faqs':
					...
					$modelCategorys = new Faqs;
					$result = $modelCategorys->toObject()->onlyNotDeleted()->get(null,array("limit"=>array("offset"=>$itens,"count"=>$limite),"order"=>" question_pt ASC"));
					
				
					$arr["grupo"] = array();
					$questionMethod = "getQuestion".System_Language::current();
					$answerMethod = "getAnswer".System_Language::current();
					...
					foreach($result as $element)
					{
						...
						$arr["grupo"][] = array(
								"id"=>$element->getId()->getVal(),
								"question" =>$element->$questionMethod()->getVal(),
								"answer"=>$element->$answerMethod()->getVal(),
								"visivel"=>1,
								"relacao"=>$element->getCategoryId()->getVal()
						);
					}
				...
				case 'usuarios':
					...
					{
						$arr["grupo"][] = array(
								"id"=>$user->getId()->getVal(),
								"nome"=>$user->getEmail()->getVal(),
								"email"=>$user->getEmail()->getVal(),
								"dt_inc"=>$user->getDateInclusion()->getVal(),
						);
					}
